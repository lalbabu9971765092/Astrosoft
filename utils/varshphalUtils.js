...
export async function calculateSolarReturnJulianDay(natalJD_UT, natalSunTropicalLongitude, targetYear) {
    ...
    const tropicalFlags = swisseph.SEFLG_SPEED; // Need speed for iteration
    ...
    try {
        for (let i = 0; i < maxIterations; i++) {
            ...
            const sunCalc = swisseph.swe_calc_ut(jdGuess, swisseph.SE_SUN, tropicalFlags);
            ...
            if (Math.abs(diff) <= tolerance) {
                logger.info(`Solar Return found at JD ${jdGuess.toFixed(6)} after ${i + 1} iterations.`);
...
export function calculateYearLord(solarReturnWeekDay, natalAscendantLord, srAscendantLord) {
    ...
    if (srAscendantLord && srAscendantLord !== "N/A" && srAscendantLord !== "Error") {
        logger.info(`Year Lord determined as SR Ascendant Lord: ${srAscendantLord}`);
        ...
        logger.info(`Year Lord determined as SR Weekday Lord (fallback): ${weekdayLord}`);
        return weekdayLord;
    } else {
